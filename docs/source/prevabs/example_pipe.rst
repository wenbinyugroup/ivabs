...
:numref:`Fig. %s <fig_pipe_draw>` [YU2005]_. This cross section has two
straight walls and two half circular walls. :math:`r=1.0` in. and other
...
:numref:`Table %s <table_pipe_materials>` and
:numref:`Table %s <table_pipe_layups>`. Cross-sectional properties are
given in :numref:`Table %s <table_pipe_result>` and compared with the
results from [YU2005]_. The tiny differences are due to different meshes. 
