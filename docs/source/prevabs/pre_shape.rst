...
.. code-block:: xml
  ...
  :caption: Base points input file syntax.
...
- **<point>** - Coordinates of the point. Two numbers are seperated by blanks.
...
- **<baseline>** - Definition of a base line (explained below).
  ...
  - *name* - Required. Name of the base line.
  - *type* - Required. Type of the base line. Choose one from 'straight', 'arc' and 'circle'.
...
- **<direction>** - Optional. Direction of the circular arc. Choose from 'cw' (clockwise) and 'ccw' (counter-clockwise). Default is 'ccw'.
...
- **<discrete>** - Number of discretization. If 'by="angle"', then new points are created every specified degrees of angle. If 'by="number"', then specified number of new points are created and evenly distributed on the arc.
  ...
  - *by* - Optional. Choose one from 'angle' and 'number'. Default is 'angle'.
