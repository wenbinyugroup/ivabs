...
#. Mode 1 runs a single analysis. This mode is only used by Dakota for each evaluation/iteration for the current version. It accepts two more arguments: name of the file containing the parameters generated by Dakota at the current evaluation/iteration (e.g., ``params.in``), and name of the file containing the result of the analysis process that will be sent back to Dakota (e.g., ``results.out``). Their names can be changed in the main input file.
