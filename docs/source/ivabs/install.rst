...
The folder ``Assets`` contains the installation files you need to download for different versions. Please discard ``Source code (zip)`` and ``Source code (tar.gz)`` because they just contain files used to generate this documentation site).
